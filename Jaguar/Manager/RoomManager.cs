...
public static class RoomManager
{
    ...
    private static T[] FindRoomsWithType<T>(uint typeId) where T : Room =>
        GetRooms<T>().Where(r => (!r.IsPlaying || r.EnableJoinAfterGameStarted) && r.Access == Access.Public
    ...
    private static T[] FindRoomsWithLevel<T>(uint level) where T : Room =>
        GetRooms<T>().Where(r => (!r.IsPlaying || r.EnableJoinAfterGameStarted)
                                 && r.Access == Access.Public && r.AllUsersJoined == false && r.Level.InRange(level) && r.Users.Count > 0)
            .ToArray();

    private static T[] FindRoomsWithTypeIdAndLevel<T>(uint level, uint typeId) where T : Room =>
        GetRooms<T>()
            .Where(r => (!r.IsPlaying || r.EnableJoinAfterGameStarted) && r.Level.InRange(level) && r.Access == Access.Public && !r.AllUsersJoined && r.Users.Count > 0 && r.TypeId == typeId)
            .ToArray();
